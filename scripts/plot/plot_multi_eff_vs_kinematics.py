...
# restrict lifetime
data = data[data["Lifetime"] > (0.0002 * scale)]
...
for m in models:
    data[f"pred_{m}"] = (
        eval_funs[m](models[m], data[cfg.features].to_numpy()) > target_cuts[m]
    )
    ...
    for name, flavor in heavy_flavors.items():
        for variable in ["Radial FD", "ETA", "PHI", "FD", "Lifetime"]:
            ...
            for m, ax in zip(models, axes):
                ...
                if ax is axes[-1]:
                    axr = ax.twinx()
                    axr.set_ylabel("Arbitrary Units", color="tab:red")
                ...
                ax.stairs(efficiencies, bins, label="efficiency", color="tab:blue")
                ax.errorbar(centers, efficiencies, yerr=uncertainties, fmt="none", color="tab:blue")
                for bar in patches:
